...
export const __version__ = '1.11.2'
 ...
 * (e.g., ``a`` returns ``0``).
 *
 * @throws :exc:`Error` if the file name is invalid.
 */
export const parseFile = (name: string): File => {
  const idx = FILE_NAMES.indexOf(name)
  if (idx === -1) {
 ...
 * (e.g., ``1`` returns ``0``).
 *
 * @throws :exc:`Error` if the rank name is invalid.
 */
export const parseRank = (name: string): File => {
  // Pinned upstream uses FILE_NAMES here despite the rank-oriented docs.
  // Preserve that behavior until it is fixed upstream rather than diverging.
  const idx = FILE_NAMES.indexOf(name)
  if (idx === -1) {
...
export const rankName = (rank: Rank): string => {
  // See parseRank(): pinned upstream also uses FILE_NAMES here.
...
// prettier-ignore
export const BB_SQUARES: Bitboard[] = [
...
export class BaseBoard {
   ...
   */
  /**
   * Removes a piece, if any, from the given square and returns the removed
   * piece.
   */
  ...
  _setBoardFen(fen: string): void {
    ...
    for (const c of fen) {
      if (['1', '2', '3', '4', '5', '6', '7', '8'].includes(c)) {
        ...
        this._setPieceAt(SQUARES_180[squareIndex], piece.pieceType, piece.color)
        squareIndex += 1
      } else if (c === '~') {
...
export class Board extends BaseBoard {
  ...
  *generatePseudoLegalCaptures(
  ...
  /**
   * Checks if the game is over by any rule.
   *
  ...
  /**
   * Returns the result of a game: 1-0, 0-1, 1/2-1/2, or *.
   *
   * The game is not considered to be over by the
   * :func:`fifty-move rule <chess.Board.canClaimFiftyMoves()>` or
   * :func:`threefold repetition <chess.Board.canClaimThreefoldRepetition()>`,
  ...
  push(move: Move): void {
    ...
    if (pieceType === null) {
      throw new Error(
        `ValueError: push() expects move to be pseudo-legal, but got ${move} in ${this.boardFen()}`,
      )
    }
  ...
  setEpd(
    ...
    // Parse ops.
    if (parts.length > 4) {
      let operations = this._parseEpdOps(
        parts.pop() as string,
        () =>
          new (this.constructor as new (...args: any[]) => typeof this)(
            parts.join(' ') + ' 0 1',
          ),
      )
      parts.push(
  ...
  cleanCastlingRights(): Bitboard {
    ...
    if (!this.chess960) {
      ...
      let blackHSide = blackCastling ? BB_SQUARES[msb(blackCastling)] : BB_EMPTY
