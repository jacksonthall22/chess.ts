...
from transpile_python_chess_tests import sync_generated_tests
...
UPSTREAM_COMMIT = "e88e7f0531b39b70fcd42e000d472f26a39ed847"

EXPECTED_GIT_BLOBS = {
    "chess/__init__.py": "b74d710fd27123a4bcb358cdbbdf13f01a759e33",
    "chess/engine.py": "c66bc0c4582d8a8d2953488ede4135bbdb081263",
    "chess/gaviota.py": "7152a18f026524a36bb3ec5d54c7799a7cc3a809",
    "chess/pgn.py": "3d4d7e5ff8b4eb3af6c16448007888436b1208f6",
    "chess/polyglot.py": "a7d6807c4f912ab16d70b27648680d8f6f318957",
    "chess/svg.py": "7e8facf99b22ab7b07c6413bfacc92dd3479f926",
    "chess/syzygy.py": "2250db5b5f7375cc180138ec0d8836b38bf7dede",
    "chess/variant.py": "ba4c0f1cee85f323771c08942062e58b505df43f",
    "test.py": "ac9066385c72d38727f5cfc6630e29157e90a95d",
...
def extract_upstream_tests() -> list[UpstreamTest]:
    tree = ast.parse(
        UPSTREAM_TEST.read_text(encoding="utf-8"), filename=str(UPSTREAM_TEST)
    )
    tests = []
...
def translated_source_lines() -> dict[int, TranslatedTest]:
    ...
    test_paths = [
        path
        for path in sorted(TYPESCRIPT_TEST_ROOT.rglob("*.test.ts"))
...
def render_todos(tests: list[UpstreamTest], translated_lines: set[int]) -> str:
    ...
    lines.extend(
        [
            "",
            f"// Baseline: python-chess {UPSTREAM_COMMIT}",
...
def main() -> None:
    ...
    if args.check:
        actual = (
            GENERATED_TODOS.read_text(encoding="utf-8")
            if GENERATED_TODOS.exists()
            else ""
        )
        if actual != expected:
            raise SystemExit(
        ...
        GENERATED_TODOS.write_text(expected, encoding="utf-8", newline="\n")

    print(
        f"python-chess {UPSTREAM_COMMIT}: {len(translated)}/{len(upstream_tests)} "
        f"tests translated ({generated_count} generated from the Python AST)"
    )
